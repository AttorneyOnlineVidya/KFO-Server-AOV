...
def ooc_cmd_rps(client, arg):
    ...
    if arg.lower() in ["clear", "cancel"]:
        if client.rps_choice:
            client.area.broadcast_ooc(f'{client.char_name} no longer wants to play Rock Paper Scissors...')
        client.rps_choice = ""
    ...
    msg += f'\n  ◽ {target.char_name} picks {target.rps_choice}!'
    msg += f'\n  ◽ {client.char_name} picks {client.rps_choice}!'
